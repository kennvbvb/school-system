...
| budget ledger แบบ append-only                | ใช้งานได้ — กันยอด โอนงบ ย้อนรายการ และห้ามยอดติดลบ                     |
| รายการจัดซื้อจัดจ้าง (ฉบับร่าง)              | ใช้งานได้ — สร้าง แก้ไข และดูรายละเอียดได้                              |
| **หน้าจอข้อมูลพื้นฐานและบัญชีงบ**            | **ใช้งานได้** — ตั้งค่าครบทั้งวงจรผ่านหน้าเว็บได้แล้ว                   |
| **ทะเบียนผู้ขาย**                            | **ใช้งานได้** — เพิ่ม แก้ไข ปิดใช้ พร้อมเตือนและกันการบันทึกซ้ำ         |
| **ตรวจลำดับเวลาและความครบถ้วน**              | **ใช้งานได้** — ปิด F-03 และ F-04 บังคับที่ฐานข้อมูล                    |
| **สายอนุมัติ**                               | **ใช้งานได้** — ตรวจสอบ อนุมัติ ส่งกลับ ยกเลิก พร้อมประวัติที่แช่แข็ง   |
| **การกันยอดงบตอนอนุมัติ**                    | **ใช้งานได้** — อนุมัติแล้วกันยอดทันที ยกเลิกแล้วคืนยอด กันงบติดลบ      |
| **เลขที่เอกสาร**                             | **ใช้งานได้** — โรงเรียนกำหนดเลขเอง ระบบกันเลขซ้ำและเลขที่ยกเลิกแล้ว    |
| **จัดการผู้ใช้และสิทธิ์**                    | **ใช้งานได้** — เชิญผู้ใช้ทางอีเมล แก้บทบาท ปิดบัญชี กันการล็อกทุกคนออก |
| **การเบิกจ่าย**                              | **ใช้งานได้** — จ่ายแล้วแปลงยอดที่กันไว้เป็นค่าใช้จ่ายจริง ยกเลิกได้    |
| **การผูกพันงบ**                              | **ใช้งานได้** — ออกใบสั่งซื้อแล้วยอดเปลี่ยนจากกันไว้เป็นผูกพัน          |
| **หน้าดูประวัติการใช้งาน (audit log)**       | **ใช้งานได้** — กรองตามประเภท ผู้กระทำ และช่วงวันที่ อ่านอย่างเดียว     |
| **รายงานงบประมาณ**                           | **ใช้งานได้** — รวมยอดตามโครงการ แหล่งเงิน ฝ่ายงาน และดูยอด ณ วันที่ได้ |
| **ทะเบียนจัดซื้อจัดจ้าง**                    | **ใช้งานได้** — ทั้งเล่มในหน้าเดียว พร้อมยกแถวที่ข้อมูลยังไม่ครบขึ้นมา  |
| **รายงานสถานะเอกสาร**                        | **ใช้งานได้** — เลขที่ขาดช่วง เลขลำดับซ้ำ และเอกสารที่ยังไม่ได้เลข      |
| **ส่งออกรายงานเป็น XLSX/CSV**                | **ใช้งานได้** — ทั้งสามรายงานข้างต้น พร้อม export log และ checksum      |
| แม่แบบและการสร้าง PDF                        | ยังไม่มี ([ADR 0009](docs/decisions/0009-legal-content-versioning.md))  |
| คลังวัสดุและครุภัณฑ์                         | ยังไม่มี (PR-07)                                                        |

**ลำดับการตั้งค่าก่อนใช้งานจริง** (ทำผ่านหน้าเว็บได้ทั้งหมดแล้ว):
ปีงบประมาณ → แหล่งเงิน → โครงการ → บัญชีงบ → จัดสรรวงเงิน → สร้างรายการจัดซื้อ
เริ่มที่ `/admin/master-data` ซึ่งเรียงขั้นตอนไว้ตามลำดับที่ต้องกรอกจริง

ผู้ขายเพิ่มได้ตลอดเวลาที่ `/admin/master-data/vendors` ไม่ต้องมีก่อนสร้างรายการจัดซื้อ
เพราะบางวิธีจัดหายังไม่มีผู้ขาย ณ วันที่ทำรายงานขอซื้อ

ลำดับงานถัดไป (PR-05 เป็นต้นไป) และเกณฑ์ตรวจรับแต่ละ PR อยู่ใน
