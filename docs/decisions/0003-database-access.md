...
- ต้องมี test ที่ยืนยันว่า `supabase/seed-reference.sql` กับ `src/domain/auth/permissions.ts` ไม่หลุดจากกัน
  → `tests/unit/seed-consistency.test.ts` ทำหน้าที่นี้ โดยไม่ต้องยกฐานข้อมูลขึ้นใน CI
- seed แยกเป็นสองไฟล์ตามขอบเขตการใช้งาน ไม่ใช่ไฟล์เดียวที่ปนกัน:
  `seed-reference.sql` เป็นข้อมูลอ้างอิงที่ผูกกับโค้ดและต้องรันทุก environment
  ส่วน `seed-sample.sql` เป็นข้อมูลสมมติที่ห้ามรันบน Production
  การรวมไว้ไฟล์เดียวเคยทำให้ต้องอาศัยการอ่านคอมเมนต์เพื่อรู้ว่าควรรันถึงบรรทัดไหน
  ซึ่งพลาดได้ง่ายและพลาดแล้วรู้ตัวช้า — ข้ามส่วนแรกแล้วระบบสิทธิ์เงียบ ๆ ไม่ทำงาน
  หรือเผลอรันส่วนหลังแล้วได้หน่วยงานสมมติติดไปอยู่ในระบบจริง
